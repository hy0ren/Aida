...
export default function ProviderPatientsScreen() {
  ...
  const [notes, setNotes] = useState(demoData.healthSummary.notesForAida);
  const highlightedMetrics = demoData.biometricMetrics.filter((metric) => metric.status === "attention");
      ...
      <View style={{ gap: 16, paddingBottom: 86 }}>
        <Card>
          <View style={{ flexDirection: "row", alignItems: "center", gap: 14 }}>
            ...
            <View style={{ flex: 1 }}>
              <Text style={{ color: theme.ink, fontSize: 19, fontWeight: "900" }}>{demoData.patient.name}</Text>
              <Text style={{ color: theme.muted, marginTop: 3 }}>DOB {demoData.patient.dob} - {language} preferred</Text>
            </View>
          </View>
          <View style={{ flexDirection: "row", gap: 8, flexWrap: "wrap", marginTop: 14 }}>
            <Pill label={`${demoData.insurance.carrier} ${demoData.insurance.plan}`} icon="card-account-details" />
            <Pill label={demoData.insurance.estimatedCopay} icon="cash" tone={colors.green} />
            <Pill label={demoData.providerIntake.intakeStatus} icon="alert-circle-outline" tone={colors.amber} />
          </View>
        </Card>
        ...
        <Card>
          ...
          <MetricRow label={demoData.biometricMetrics[4].label} value={`${demoData.biometricMetrics[4].value}%`} tone={colors.green} />
        </Card>
